  ...
  btn.onclick = () => {
    if (confirm('Reset all responses for this activity? This cannot be undone.')) {
      Object.keys(localStorage)
        .filter(k => k.endsWith('_selected_t1') || k.endsWith('_correct_t1') || k.endsWith('_result_t1'))
...
+ Variables which take on numerical values <u>and</u> for which measures such as the mean, median, or standard deviation are meaningful are referred to as **numerical**. 
...
viewof q1 = Inputs.checkbox(
  ...
  {value: JSON.parse(localStorage.getItem("q1_selected_t1") ?? "[]") ?? []}
);

{
  localStorage.setItem("q1_selected_t1", JSON.stringify(q1));
  localStorage.setItem("q1_correct_t1", "3,4,5");
  localStorage.setItem("q1_result_t1", (!q1 || q1.length === 0) ? "unattempted" : (q1.toString() === "3,4,5" ? "correct" : "incorrect"));
}
ok_checkbox(q1, "3,4,5");
```

:::

:::{.callout-tip}
## The `month` Variable as Non-Numeric
...
viewof q2 = Inputs.checkbox(
  ...
  {value: JSON.parse(localStorage.getItem("q2_selected_t1") ?? "[]") ?? []}
);

{
  localStorage.setItem("q2_selected_t1", JSON.stringify(q2));
  localStorage.setItem("q2_correct_t1", "2,6,7");
  localStorage.setItem("q2_result_t1", (!q2 || q2.length === 0) ? "unattempted" : (q2.toString() === "2,6,7" ? "correct" : "incorrect"));
}
ok_checkbox(q2, "2,6,7");
```
...
viewof q3 = Inputs.checkbox(
  ...
  {value: JSON.parse(localStorage.getItem("q3_selected_t1") ?? "[]") ?? []}
);

{
  localStorage.setItem("q3_selected_t1", JSON.stringify(q3));
  localStorage.setItem("q3_correct_t1", "1");
  localStorage.setItem("q3_result_t1", (!q3 || q3.length === 0) ? "unattempted" : (q3.toString() === "1" ? "correct" : "incorrect"));
}
ok_checkbox(q3, "1");
```
...
#| eval: true
#| results: false
#| fig-alt: "Three-panel figure. Top-Left panel: A scatter plot of diamond prices against carat weight -- as carat weight increases, price generally increases in a non-linear fashion. Top-Right panel: A scatter plot of carat weight against diamond depth -- there does not seem to be a pattern in the organization of the points. Bottom panel: A scatter plot of diamond width against diamond length, each in millimeters -- the vast majority of the points in the plot lie along a slightly upward sloping line."
...
viewof q4 = Inputs.checkbox(
  ...
  {value: JSON.parse(localStorage.getItem("q4_selected_t1") ?? "[]") ?? []}
);

{
  localStorage.setItem("q4_selected_t1", JSON.stringify(q4));
  localStorage.setItem("q4_correct_t1", "1,3");
  localStorage.setItem("q4_result_t1", (!q4 || q4.length === 0) ? "unattempted" : (q4.toString() === "1,3" ? "correct" : "incorrect"));
}
ok_checkbox(q4, "1,3");
```
...
viewof q5 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q5_selected_t1") ?? "null")}
);

{
  localStorage.setItem("q5_selected_t1", JSON.stringify(q5));
  localStorage.setItem("q5_correct_t1", "2");
  localStorage.setItem("q5_result_t1", q5 === null ? "unattempted" : (q5 == 2 ? "correct" : "incorrect"));
}
ok_response(q5, "2");
```
...
viewof q6 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q6_selected_t1") ?? "null")}
);

{
  localStorage.setItem("q6_selected_t1", JSON.stringify(q6));
  localStorage.setItem("q6_correct_t1", "3");
  localStorage.setItem("q6_result_t1", q6 === null ? "unattempted" : (q6 == 3 ? "correct" : "incorrect"));
}
ok_response(q6, "3");
```
...
#| warning: false
#| results: false
#| fig-alt: "Two-panel figure comparing home price and exterior covering by waterfront status. Left panel: boxplot of price (log scale) for non-waterfront versus waterfront homes. Non-waterfront homes have a lower median price, around $400,000, with a tight interquartile range and many high-value outliers extending above $3,000,000. Waterfront homes have a notably higher median price, around $1,300,000, with less spread and fewer extreme outliers. Right panel: stacked bar chart of the proportion of exterior covering type (brick, shingles, vinyl siding, or missing) for non-waterfront and waterfront homes. The two distributions are nearly identical, with vinyl siding making up roughly 70% of homes in both groups, followed by smaller shares of brick and shingles."
...
viewof q7 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q7_selected_t1") ?? "null")}
);

{
  localStorage.setItem("q7_selected_t1", JSON.stringify(q7));
  localStorage.setItem("q7_correct_t1", "1");
  localStorage.setItem("q7_result_t1", q7 === null ? "unattempted" : (q7 == 1 ? "correct" : "incorrect"));
}
ok_response(q7, "1");
```
...
#| echo: false
#| eval: true
#| fig-alt: "A scatterplot of randomly positioned points in space, representing a population. All points are colored orange, indicating that they are all included in the sample. When all elements of the population are sampled, it is called a Census."
...
#| echo: false
#| eval: true
#| fig-alt: "Another scatterplot representing a hypothetical population. This time, the majority of points are colored blue and only about 10% are colored orange. The orange points are selected randomly for inclusion in a sample. This picture represents the results of taking a simple random sample."
...
![](images/Stratified.png){fig-alt="Another scatterplot representing a hypothetical population, but this time observations only exist within six (6) circles labeled stratum. Within each of the stratum, the majority of points are colored blue and a small random sample is colored orange. This image shows the results of taking a stratified sample."}
...
![](images/cluster.png){fig-alt="A scatterplot representing a hypothetical population, where observations exist within nine circles, each representing a cluster. A random sample of the clusters is selected, outlined in orange, and then within each selected cluster a random sample of observations is taken and colored orange; the remaining observations are blue. This picture shows the results of a multi-stage cluster sample. Those elements of the population included in the sample are orange, while those excluded are blue."}

:::

::::

The ***convenience sample*** is the most commonly used sampling method. Unfortunately, it is also the worst. When researchers sample from individuals they have "easy access" to, they are conducting a convenience sample. There are always hidden biases in these samples. [Here's a famous example](https://youtu.be/JwZo28RKdvU){target="_blank"} in which the Literary Digest incorrectly predicted a landslide victory for Alf Landon over FDR in the 1936 US Presidential Election. 
...
There's lots more to learn about experimental design, but it is beyond the scope of this notebook. You should read pages 32 through 35 of [OpenIntro Statistics, 4Ed](https://www.openintro.org/book/os/){target="_blank"} as a starting point.
...
function buildQuestionResults() {
  return {
    notebook: "Topic 1: An Introduction to Data and Sampling",
    type: "questions",
    timestamp: new Date().toISOString(),
    questions: {
      q1_numerical_variables: {
        selected: q1,
        correct_answer: "3,4,5",
        result: (!q1 || q1.length === 0) ? "unattempted" : (q1.toString() === "3,4,5" ? "correct" : "incorrect")
      },
      q2_categorical_variables: {
        selected: q2,
        correct_answer: "2,6,7",
        result: (!q2 || q2.length === 0) ? "unattempted" : (q2.toString() === "2,6,7" ? "correct" : "incorrect")
      },
      q3_unique_identifiers: {
        selected: q3,
        correct_answer: "1",
        result: (!q3 || q3.length === 0) ? "unattempted" : (q3.toString() === "1" ? "correct" : "incorrect")
      },
      q4_associations: {
        selected: q4,
        correct_answer: "1,3",
...
[1] House Sales in King County, USA (2016). Kaggle. Retrieved from [Kaggle House Sales Dataset](https://www.kaggle.com/datasets/harlfoxem/housesalesprediction).
