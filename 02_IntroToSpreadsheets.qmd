...
+ If you find yourself frustrated, take a break. A fresh set of eyes can often reveal an error that was staring you in the face for twenty minutes.
...
You can also combine these in one formula: `=1+1-(5*2)+(8/4)` evaluates to `-6`.
...
The numerator is `54 - 50`. The denominator is `8` divided by `√20`. Use `SQRT()` for the square root. Your formula will have the shape `=(54-50)/(8/SQRT(___))`
...
+ **Absolute references** (`$B$1`) — use these when one cell holds a fixed value (like a tax rate, a sample size, or a probability) that every formula in your table should use. Without the dollar signs, copying the formula will accidentally shift away from that fixed value and give you wrong answers.
:::
...
Note that if you click into the cell **D2** and drag the solid square in the lower-right corner of the cell down, you'll get the results of the same calculation over and over again. If you try this same thing, but omit the dollar signs from your formula, you'll see that the cell reference locations update and the results of your formula change from one row to the next.
...
For example, if cell **B1** contains `=AVERAGE(A1:A6)` and you copy it to **C1**, it becomes `=AVERAGE(B1:B6)`, which is probably what you intended. But if you accidentally copy it to **B2**, it becomes `=AVERAGE(A2:A7)`, silently shifting your range by one row and dropping the first value.
...
+ Each **row** represents an **observation** (one subject or case).
+ The **first row** typically contains column headers — short labels that name each variable.

Check out [this example data table containing data on diamonds](https://docs.google.com/spreadsheets/d/1SGVef3nVZ4Sg34U4rfudJgLV0eOCcgg1EnAsVecR1IU/edit?usp=sharing){target="_blank"}. This is exactly the same structure as what a programmer would call a *data frame*. If you have ever used a data file in Excel or Sheets, you have already worked with a data table, even if you didn't call it that.
...
2. **Typing a specific range** like `=AVERAGE(A2:A101)` that covers exactly your data rows (excluding the header in row 1). This is more precise and is generally the better habit.
...
+ A **data table** in a spreadsheet has the same structure as a data frame: columns are variables, rows are observations, and the first row contains column headers.
