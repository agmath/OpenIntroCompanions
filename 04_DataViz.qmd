...
Use some of the same functions you used to explore the `mpg` data frame in the previous code block — just swap out the data frame name.
...
Four things to update: change the geometry layer to `geom_bar()`, map `class` to `x`, remove the `y` aesthetic (bar heights are determined by counts automatically), and map `drv` to `fill`.
...
+ **Variable types dictate plot types.** Always consider whether your variables are numerical or categorical before choosing a plot. A scatterplot requires two numerical variables; a bar plot is appropriate for a single categorical variable; side-by-side boxplots or bar plots with fill work for numerical vs. categorical comparisons.
...
The next activity introduces descriptive statistics — computing and interpreting numerical summaries for both numerical and categorical data. You'll use the R tools introduced here alongside `dplyr` functions like `summarize()` and `group_by()` to compute means, medians, standard deviations, and frequency tables.
:::
