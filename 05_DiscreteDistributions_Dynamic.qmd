...
If the first question that we get wrong is question 3, what must have been the outcome for each of the first two questions?
...
If the first question that we get wrong is question 3, what must have been the outcome for each of the first two questions? We must have gotten both of the first two questions right. The probability of getting any individual question right was 0.20.
...
We must have gotten the third question wrong. The probability of guessing incorrectly was 0.80.
...
That call removes the probability of at most `{ojs} thresh_atleast` successes, leaving only the probability of *more than* `{ojs} thresh_atleast`. We want *at least* `{ojs} thresh_atleast`, which includes `{ojs} thresh_atleast` itself. Replace the first argument with `{ojs} Number(thresh_atleast) - 1`.
...
Use `{ojs} Number(thresh_atleast) - 1` as the first argument, `{ojs} n_students` as the second, and `{ojs} p_assign` as the third.
...
Use `{ojs} thresh_between_hi` and `{ojs} Number(thresh_between_lo) - 1` as the first arguments of the two `pbinom()` calls, with `{ojs} n_students` and `{ojs} p_assign` as the remaining arguments in both.
