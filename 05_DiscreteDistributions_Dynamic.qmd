...
That call calculates the probability of 0 through `{ojs} thresh_atleast` successes. That's not what we want. We want `{ojs} thresh_atleast` *or more*.
...
That subtracts the probability of at most `{ojs} thresh_between_lo` successes, which *excludes* `{ojs} thresh_between_lo` itself from the answer. We want to *include* `{ojs} thresh_between_lo`, so replace the first argument of that second call to `pbinom()` with `{ojs} thresh_between_lo - 1`.
...
+ If $X$ counts successes in a binomial experiment with $n$ trials and success probability $p$:
  ...
  + Draw a picture to help you see how to use `pbinom()` and/or `dbinom()` to calculate probabilities. These two functions above are sufficient to handle obtain binomial probability — the challenge is identifying how to combine them.
