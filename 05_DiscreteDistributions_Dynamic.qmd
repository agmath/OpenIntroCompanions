...
# "at most" threshold: somewhere around the median
thresh_atmost <- round(n_students * p_assign) + sample(1:3, 1)

# "at least" threshold: upper portion of the distribution
thresh_atleast <- round(n_students * p_assign) + sample(4:7, 1)
thresh_atleast <- min(thresh_atleast, n_students - 1)  # keep in range

# "between" thresholds: lo below median, hi above median
...
#a correct response on one question is 0.2
dbinom(2, 4, 0.2)
```
...
If we had 100 randomly chosen students, how many might you expect failed to hand in at least one assignment? 

How dd you know that? What, mathematically, did you do?
...
The second argument is the total number of trials. There are `{ojs} n_students` students in the sample, so fill in the second blank with `{ojs} n_students`.
...
For `pbinom()`, the first argument is the maximum number of successes you are willing to consider — that's `{ojs} thresh_atmost` here. The second and third arguments are the number of trials and the probability of success.
...
The approach below, where the blanks are `{ojs} thresh_atleast`, `{ojs} n_students`, and `{ojs} p_assign`, results in the wrong probability — why?
...
That call calculates the probability of 0 successes, 1 success, ..., up to `{ojs} thresh_atleast` successes. That's not what we want. We want `{ojs} thresh_atleast` *or more*.
...
That call removes the probability of at most `{ojs} thresh_atleast` successes, leaving only the probability of more than `{ojs} thresh_atleast` successes. We want *at least* `{ojs} thresh_atleast`, which includes `{ojs} thresh_atleast` itself. Replace the first argument with `{ojs} thresh_atleast - 1`.
...
Which of the following corresponds to a probability that is definitely too big — filling the first argument of `pbinom()` with `{ojs} thresh_between_lo` or filling it with `{ojs} thresh_between_hi`?
...
Grading function is not currently available for these web-based notebooks. If you would like to utilize grading functionality, please use either the [Posit Cloud pre-built instance](https://posit.cloud/content/6328402) or [install the `{learnr}` notebooks locally](https://agmath.github.io/IntroductoryStatistics/AccessingInteractiveNotes.html).
