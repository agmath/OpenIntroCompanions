...
In this lab, we examine height and weight data. We explore what is meant by the *near-normality* assumption, we work towards assessing normality via a variety of informal methods, and we also investigate what the consequences can be when this assumption is violated.
  ...
  btn.onclick = () => {
    if (confirm('Reset all responses for this activity? This cannot be undone.')) {
      Object.keys(localStorage)
        .filter(k => k.endsWith('_selected_t8') || k.endsWith('_correct_t8') || k.endsWith('_result_t8'))
...
You'll see that for every observation we have 25 measurements, many of which are either diameters or girths. A complete *data dictionary* (a general description along with definitions of each of the columns/variables) can be found [here at OpenIntro](https://www.openintro.org/book/statdata/?data=bdims){target="_blank"}, but we'll be focusing on just three columns in this activity: weight in kg (`wgt`), height in cm (`hgt`), and `sex` (`1` indicates male, `0` indicates female).

Since males and females tend to have different body dimensions, it will be useful to create two additional data sets: one with only men and another with only women. Again, in the `bdims` dataset females are denoted by a 0 in the `sex` column while males are denoted by a 1. 

:::{.callout-note}
## Binary Encodings and Order

The choice to encode females by 0 and males by 1 is made because *f*emale comes before *m*ale, alphabetically. Generally, in the case of binary variables, the first category in alphabetical order is encoded by 0 and the second is encoded by 1. Note, however, that this choice can be (and often is) overridden.
:::

The code block below is set up to create a dataset `mdims` consisting of only males. Add the code necessary to create a dataset of only females called `fdims`.
...
Females are encoded in this data set by the value 0 under the `sex` column. 
...
viewof q1 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q1_selected_t8") ?? "null")}
);

{
  localStorage.setItem("q1_selected_t8", JSON.stringify(q1));
  localStorage.setItem("q1_correct_t8", "1");
  localStorage.setItem("q1_result_t8", q1 === null ? "unattempted" : (q1 == 1 ? "correct" : "incorrect"));
}
ok_response(q1, "1");
```
...
viewof q2 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q2_selected_t8") ?? "null")}
);

{
  localStorage.setItem("q2_selected_t8", JSON.stringify(q2));
  localStorage.setItem("q2_correct_t8", "1");
  localStorage.setItem("q2_result_t8", q2 === null ? "unattempted" : (q2 == 1 ? "correct" : "incorrect"));
}
ok_response(q2, "1");
```
...
viewof q3 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q3_selected_t8") ?? "null")}
);

{
  localStorage.setItem("q3_selected_t8", JSON.stringify(q3));
  localStorage.setItem("q3_correct_t8", "1");
  localStorage.setItem("q3_result_t8", q3 === null ? "unattempted" : (q3 == 1 ? "correct" : "incorrect"));
}
ok_response(q3, "1");
```
...
You may have been surprised or frustrated that the activity suggests that the heights are approximately normally distributed. It is very difficult to assess normality directly from a histogram. Let's dig a bit deeper.
...
ggplot() + 
  geom_histogram(data = fdims,
                 aes(x = hgt, y = after_stat(density))) + 
  geom_line(aes(x = seq(140, 200, length.out = 200), 
                y = dnorm(seq(140, 200, length.out = 200), 
                          mean = fhgtmean, sd = fhgtsd)),
            color = "blue", lwd = 1.25) +
  labs(title = "Distribution of Female Heights", 
       x = "Height (cm)", 
       y = "Proportion")
```
...
We've generally seen our data frames being piped into `ggplot()`. This works great when every plot layer utilizes the same data. In cases where plot layers use different data, it is necessary to pass the `data` argument directly to the plot layer in the way we did above. This allows greater flexibility in your plots.
:::
...
A data set that is nearly normal will result in a normal probability plot where the points closely follow the line. Any deviations from normality lead to deviations of these points from the line. 
:::
...
viewof q4 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q4_selected_t8") ?? "null")}
);

{
  localStorage.setItem("q4_selected_t8", JSON.stringify(q4));
  localStorage.setItem("q4_correct_t8", "2");
  localStorage.setItem("q4_result_t8", q4 === null ? "unattempted" : (q4 == 2 ? "correct" : "incorrect"));
}
ok_response(q4, "2");
```
...
**Question 1:** What is the probability that a randomly chosen young adult female is taller than 6 feet (about 182 cm)? Use the code block below to answer this question assuming that female heights are normally distributed. Use the `pnorm()` function we were introduced to in a recent activity.
...
+ **Visual tools help assess normality, but require judgment.** Overlaying a normal curve on a relative frequency histogram gives you a sense of how closely your data follows the normal model, but deciding "close enough" in this way is inherently subjective.
...
+ **Comparing to simulated normal data helps calibrate your judgment.** The `qqnormsim()` function generates Q-Q plots from data known to be normal, giving you a reference for what "close enough" actually looks like in practice.
    + If you want to use this function, you'll need to install and load the `{openintro}` library though.
+ **The normality assumption has real consequences.** When the assumption is reasonable — as with female heights — theoretical probabilities computed from the normal model agree closely with empirical probabilities. When it is not — as with female weights — the theoretical probabilities can be quite far off.
+ **Always check your assumptions before applying the normal model.** The tools introduced in this lab give you several informal means to do so. Using the normal model blindly, without verifying approximate normality, can lead to conclusions that simply aren't trustworthy.
:::
