...
This activity introduces Analysis of Variance (ANOVA) for comparing means across more than two groups simultaneously. We'll test whether average petal width differs across three varieties of iris using the famous `iris` dataset collected by Anderson and published by Fisher. We'll also introduce the Tukey Honestly Significant Differences test for pairwise follow-up comparisons.
  ...
  btn.onclick = () => {
    if (confirm('Reset all responses for this activity? This cannot be undone.')) {
      Object.keys(localStorage)
        .filter(k => k.endsWith('_selected_t18') || k.endsWith('_correct_t18') || k.endsWith('_result_t18'))
...
+ ANOVA tests whether there is significant evidence of a difference in means across **three or more** groups.
...
As we've done with previous datasets, you can simply call the name of the data frame or pipe it into functions like `head()`, `dim()`, `glimpse()`, or `skim()`.
...
viewof q1 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q1_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q1_selected_t18", JSON.stringify(q1));
  localStorage.setItem("q1_correct_t18", "3");
  localStorage.setItem("q1_result_t18", q1 === null ? "unattempted" : (q1 == 3 ? "correct" : "incorrect"));
}
ok_response(q1, "3");
```
...
viewof q2 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q2_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q2_selected_t18", JSON.stringify(q2));
  localStorage.setItem("q2_correct_t18", "3");
  localStorage.setItem("q2_result_t18", q2 === null ? "unattempted" : (q2 == 3 ? "correct" : "incorrect"));
}
ok_response(q2, "3");
```
...
viewof q3 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q3_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q3_selected_t18", JSON.stringify(q3));
  localStorage.setItem("q3_correct_t18", "2");
  localStorage.setItem("q3_result_t18", q3 === null ? "unattempted" : (q3 == 2 ? "correct" : "incorrect"));
}
ok_response(q3, "2");
```
...
viewof q4 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q4_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q4_selected_t18", JSON.stringify(q4));
  localStorage.setItem("q4_correct_t18", "3");
  localStorage.setItem("q4_result_t18", q4 === null ? "unattempted" : (q4 == 3 ? "correct" : "incorrect"));
}
ok_response(q4, "3");
```
...
viewof q5 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q5_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q5_selected_t18", JSON.stringify(q5));
  localStorage.setItem("q5_correct_t18", "1");
  localStorage.setItem("q5_result_t18", q5 === null ? "unattempted" : (q5 == 1 ? "correct" : "incorrect"));
}
ok_response(q5, "1");
```
...
viewof q6 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q6_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q6_selected_t18", JSON.stringify(q6));
  localStorage.setItem("q6_correct_t18", "3");
  localStorage.setItem("q6_result_t18", q6 === null ? "unattempted" : (q6 == 3 ? "correct" : "incorrect"));
}
ok_response(q6, "3");
```
...
viewof q7 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q7_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q7_selected_t18", JSON.stringify(q7));
  localStorage.setItem("q7_correct_t18", "4");
  localStorage.setItem("q7_result_t18", q7 === null ? "unattempted" : (q7 == 4 ? "correct" : "incorrect"));
}
ok_response(q7, "4");
```
...
viewof q8 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q8_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q8_selected_t18", JSON.stringify(q8));
  localStorage.setItem("q8_correct_t18", "1");
  localStorage.setItem("q8_result_t18", q8 === null ? "unattempted" : (q8 == 1 ? "correct" : "incorrect"));
}
ok_response(q8, "1");
```
...
viewof q9 = Inputs.radio(
  ...
  {value: JSON.parse(localStorage.getItem("q9_selected_t18") ?? "null")}
);

{
  localStorage.setItem("q9_selected_t18", JSON.stringify(q9));
  localStorage.setItem("q9_correct_t18", "1");
  localStorage.setItem("q9_result_t18", q9 === null ? "unattempted" : (q9 == 1 ? "correct" : "incorrect"));
}
ok_response(q9, "1");
```
...
The ANOVA result tells us that *at least one variety has a different average petal width* — but it doesn't tell us which one. The **Tukey Honestly Significant Differences (HSD) Test** can fill that gap. When ANOVA returns a significant result, the Tukey HSD Test runs a collection of pairwise comparisons with adjusted $p$-values that account for the multiple comparisons being made.

Since the result from our ANOVA test was significant, we'll follow up with a Tukey HSD Test. In R, we pass the result of `aov()` directly to `TukeyHSD()`. Run the code block below and use the output to answer the question that follows.
...
viewof q10 = Inputs.checkbox(
  ...
  {value: JSON.parse(localStorage.getItem("q10_selected_t18") ?? "[]") ?? []}
);

{
  localStorage.setItem("q10_selected_t18", JSON.stringify(q10));
  localStorage.setItem("q10_correct_t18", "1,2,3");
  localStorage.setItem("q10_result_t18", (!q10 || q10.length === 0) ? "unattempted" : (q10.toString() === "1,2,3" ? "correct" : "incorrect"));
}
...
If you'd like to verify the results contained in your hashes, you can [do so here](https://agmath.github.io/OpenIntroCompanions/hash_check.html).
...
The final activity in this series is a regression lab. We'll shift from comparing group means to modeling the relationship between two numerical variables — using one to predict (or explain) the other. Linear regression extends the inference framework you've built throughout this course into a new and widely-used context.
:::
