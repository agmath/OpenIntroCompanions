...
gradethis::grade_this({
  if (.result == .solution) {
    eval.js("localStorage.setItem('ex_mean_sample_one', 'pass')")
    pass(random_praise())
  }
  eval.js("localStorage.setItem('ex_mean_sample_one', 'fail')")
...
gradethis::grade_this({
  if (.result == .solution) {
    eval.js("localStorage.setItem('ex_mean_sample_three', 'pass')")
    pass(random_praise())
  }
  eval.js("localStorage.setItem('ex_mean_sample_three', 'fail')")
  fail(random_encouragement())
})
```
...
gradethis::grade_this_code(
  correct = {
    eval.js("localStorage.setItem('ex_my_data', 'pass')")
    pass(random_praise())
  },
  incorrect = {
    eval.js("localStorage.setItem('ex_my_data', 'fail')")
    fail(random_encouragement())
  }
)
```
...
The two plots below show the distribution of `carat` weights for the diamonds in our population. The purple line marks the mean.
...
The code block below computes standard deviations and IQRs for each sample. Run it and explore the output. *(Not graded.)*
...
Use the `c()` function to define your data, then apply `sd()` and `IQR()`.
...
The code block below is preset for `Sample_One` — adapt it to summarize `Sample_Two` and `Sample_Three`. *(Not graded.)*
...
function buildResults() {
  return {
    ...
    exercises: {
      mean_sample_one: {
        result: localStorage.getItem("ex_mean_sample_one") ?? "unattempted"
      },
      mean_sample_three: {
        result: localStorage.getItem("ex_mean_sample_three") ?? "unattempted"
      },
      my_data: {
        result: localStorage.getItem("ex_my_data") ?? "unattempted"
      }
    }
  };
}
...
// Recompute whenever any question changes
submission_hash = {
  q1; q2; q3; q4; q5; q6;
...
html`<div style="font-family: monospace; font-size: 0.85em; background: #f5f5f5; padding: 12px; border-radius: 6px; word-break: break-all; border: 1px solid #ddd; user-select: all; cursor: pointer;" onclick="this.focus(); document.execCommand('selectAll')">
  ${submission_hash}
</div>
<p style="margin-top: 8px; font-size: 0.9em; color: #555;">
  Click the box to select, then copy (Ctrl+C / Cmd+C).
</p>`
```
