...
#| echo: true
#| eval: false
# Start with the code below (copied from our earlier code chunk)
# and update it to compute the mean of Sample_Three.
samples |>
  summarize(avg_carat_wt = mean(Sample_Two))
```
...
3. Use the `median()` function and the code block below to compute the median of each of the samples and then answer the question that follows. (*Note.* There are several ways to obtain the medians, so this code block won't evaluate you as correct/incorrect.)
...
* What are we averaging? The quantity $\left(x - \bar{x}\right)$ denotes an observed value's deviation from the mean. We shouldn't average these values though, since the mean sits in the *center* of the data and we would have deviations above the mean (positive) "cancelling out" deviations below the mean (negative).
...
* The boxplot doesn't show the detailed shape that the histogram does, but it does a great job showing the IQR, median, and any outliers present.
  ...
  * The faded purple line shows the location of the mean carat weight. We can really see the impact of those outliers on the mean here.
...
In R we can easily compute the standard deviation with the function `sd()`, and the IQR with `IQR()`, for our samples or for the entire dataset. The code block below is preset to compute the standard deviation and quartiles for each sample. Run it and explore the output. *(Not graded.)*
...
Use the code block below to find the standard deviation and IQR for the doors visited by the campaign workers. *(Not graded.)*
...
We can pipe our data frame into the `count()` function with the column we'd like to obtain counts for in order to construct a frequency table. To obtain a relative frequency table, we can use the `mutate()` function to create a new column — below, we've named it `relative_frequency` and computed its values by taking the counts (`n`) and dividing by the total count (`sum(n)`). The following code block is preset to compute a frequency and relative frequency table for `Sample_One`. Adapt the code to provide summaries for `Sample_Two` and `Sample_Three`. *(Not graded.)*
...
In the next activity we'll shift from summarizing data numerically to visualizing it. We'll learn how to construct histograms, boxplots, and bar charts in R using `ggplot2` — the same plots you saw in this activity — and develop intuition for choosing the right visualization for a given variable type and question.
:::
